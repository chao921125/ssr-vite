...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create remix-eaee
  fly apps create remix-eaee-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-eaee
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app remix-eaee-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'remix-eaee-staging' does not match app name in config file 'remix-eaee'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name remix-eaee-db
  fly postgres attach --app remix-eaee remix-eaee-db

  fly postgres create --name remix-eaee-staging-db
  fly postgres attach --app remix-eaee-staging remix-eaee-staging-db
  ```
